...
object Engine {
  ...
  implicit class MigrationsOps(val migrations: List[Migration]) extends AnyVal {
    def exec[F[_]: Applicative: Bracket[*[_], Throwable]](tx: Transactor[F],
                                                          log: Option[String => F[Unit]] = None)
                                                         (implicit F: MonadError[F, Throwable]): F[Unit] =
      for {
        _ <- log.traverse(_("DbRush - Create table if not exists db_rush_commits"))
        ...
        _ <- commitsSorted.map(Some(_)).zipAll(migrations.map(Some(_)), None, None)
          .traverse {
            case (Some(c), None) =>
              F.raiseError[Unit](new Throwable(s"DbRush - No migration matching commit $c"))
            ...
            case (Some(c), Some(m)) if c.label != m.label =>
              F.raiseError[Unit](
                new Throwable(s"DbRush - Label not matching got: '${m.label}' expected: '${c.label}'")
              )

            case (Some(c), Some(m)) if c.md5 != m.md5Hash =>
              F.raiseError[Unit](
                new Throwable(s"DbRush - Migration '${c.label}' md5 mismatch, got: '${m.md5Hash}' expected: '${c.md5}'")
              )

            case (Some(_), Some(m)) =>
              log.traverse(_(s"DbRush - Skipping migration '${m.label}'")).void
