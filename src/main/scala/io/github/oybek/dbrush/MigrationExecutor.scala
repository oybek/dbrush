...
object MigrationExecutor {
  def make[F[_] : BracketThrow](tx: Transactor[F], log: Option[String => F[Unit]] = None): MigrationExecutor[F] = new MigrationExecutor[F] {
    override def apply(migrations: List[Migration]): F[Unit] = for {
      ...
      _ <- sortedCommits.map(Some(_)).zipAll(migrations.map(Some(_)), None, None)
        .traverse_ {
          case (Some(c), None) =>
          ...
          case (Some(_), Some(m)) =>
            log.traverse_(_ (s"DbRush - Skipping migration '${m.label}'")).void
    ...
    def fail(message: String): F[Unit] = new Throwable(s"DbRush - ERROR - $message").raiseError[F, Unit]
  }
}
